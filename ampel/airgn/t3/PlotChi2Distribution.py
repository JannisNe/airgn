            ...
            for unit in units:
                for t2 in view.get_t2_views(unit=unit):
                    ...
                    if any(
                        ...
                        raw_lightcurve = datapoints_to_dataframe(dps, columns)[0]
                        ...
                        for ax in axs:
                            ax.set_yscale("log")
                        fig.suptitle(f"Transient {view.id} (unit: {unit}{std_name})")
                        fig.savefig(
                            plot_dir / f"chi2_exceed_{view.id}_{unit}{std_name}.pdf"
                        )
                        plt.close(fig)
