    ...
    def process(self, gen: Generator[T, T3Send, None], t3s: T3Store) -> UBson:
        ...
        if not self.drop_wise_agn:
            ...
            for i in range(n_splits):
                ...
                wise_target_test = target_test.iloc[wise_test_indices]
                wise_data_test = data_test[metrics].iloc[wise_test_indices]
                i_probs_wise = est.predict_proba(wise_data_test)[:, 1]
                wise_recalls.append(
                    [recall_score(wise_target_test, i_probs_wise > ix) for ix in x]
                )

                non_wise_target_test = target_test.iloc[non_wise_test_indices]
                non_wise_data_test = data_test[metrics].iloc[non_wise_test_indices]
